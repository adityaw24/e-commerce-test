...
export const ModalProductDetail = ({ show, handleModal }) => {
  ...
  const onSubmit = () => {
    (!getUserID || idUser === 0) &&
      alert("Sorry You need login before checkout");
    getUserID &&
      idUser !== 0 &&
      dispatch(createCheckout(qtyNew, totalPrice, productDetail.id, idUser));
    handleModal();
    handleRefresh();
  };
  ...
  return (
    <Modal show={show} onHide={handleModal} centered>
      ...
      <Modal.Footer>
        ...
        <Button variant="secondary" size="sm" onClick={handleModal}>
          Cancel
        </Button>
      </Modal.Footer>
    </Modal>
